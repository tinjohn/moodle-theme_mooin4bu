...
define('THEME_MOOIN4BU_SETTING_INHERITANCE_INHERIT', 0);
define('THEME_MOOIN4BU_SETTING_INHERITANCE_DUPLICATE', 1);
 ...
 * @return string
 */
function theme_mooin4bu_get_main_scss_content($theme) {
    global $CFG;
    ...
    $scss .= file_get_contents($CFG->dirroot . '/theme/mooin4bu/scss/post.scss');
 ...
 * @return string
 */
function theme_mooin4bu_get_pre_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_mooin4bu', 'prescssinheritance');
    if ($inheritanceconfig == THEME_MOOIN4BU_SETTING_INHERITANCE_DUPLICATE) {
        $scss .= theme_boost_union_get_pre_scss(theme_config::load('boost_union'));
    }

    // And add Boost Union Child's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/mooin4bu/scss/pre.scss');
 ...
 * @return string
 */
function theme_mooin4bu_get_extra_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_mooin4bu', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_MOOIN4BU_SETTING_INHERITANCE_DUPLICATE) {
 ...
 * @return array
 */
function theme_mooin4bu_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_mooin4bu'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_mooin4bu'),
        'btn' => 'primary',
        'url' => new \moodle_url('/admin/settings.php', ['section' => 'theme_mooin4bu']),
    ];
